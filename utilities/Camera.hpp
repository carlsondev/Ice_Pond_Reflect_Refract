...
public:
  ...
  /// \desc creates the View Matrix based on the position, lookAt point, and up
  /// vector
...
protected:
  ...
  /// \desc stores the View Matrix corresponding to the inverse of the Camera's
  /// Matrix
...
private:
  /// \desc keeps phi within the range (0, pi) to prevent the camera from
  /// flipping over
  void _clampPhi();
};
