...
/// \desc concrete Camera implementation to implement an ArcBall camera
/// model \note camera direction is controlled by setting theta & phi and
/// rotating the camera
class ArcballCam : public Camera {
public:
  explicit ArcballCam(GLfloat minRadius = 2.0f, GLfloat maxRadius = 30.0f)
...
inline void ArcballCam::recomputeOrientation() {
  ...
  printf("Arcball Direction (%f, %f, %f)\n", _direction.x, _direction.y,
         _direction.z);
  printf("Theta %f, Phi %f, Radius %f\n", _theta, _phi, _radius);
